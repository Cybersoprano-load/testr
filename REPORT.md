...
| node-exporter | http://localhost:9100 | `node-exporter:9100` | метрики хоста (CPU, RAM, диск, сеть) |
...
- Prometheus targets `kafka-stub`, `postgres`, `kafka`, `node` — все `up`.
- `postgres-exporter`: `pg_up = 1`. `kafka-exporter`: топик `messages` = 6 партиций.
  `node-exporter`: отдаёт `node_*` (CPU, память, диск, сеть).
- Grafana: provisioning отработал — датасорсы **Prometheus** и **InfluxDB**, дашборды
  ...
  **Apache JMeter Dashboard (Core InfluxdbBackendListenerClient)**, **Node Exporter (host)**.
...
- Мониторинг закрывает все четыре группы метрик из задания (JVM, PostgreSQL, JMeter, Kafka)
  стандартными дашбордами Grafana Labs; дополнительно добавлены хостовые метрики
  (node-exporter: CPU, RAM, диск, сеть) — отдельный дашборд «Node Exporter (host)».
